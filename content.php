...
<?php if(is_singular()): // Display Excerpts for Single/Page ?>
	...
	<div class="entry-content">
		<?php the_content(); ?>
		<?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'birdfield' ), 'after' => '</span>' ) ); ?>
	</div>
...
<?php else: // Display Excerpts for Home, Search, Archive ?>
	...
	<footer class="entry-meta">
		<span class="postdate" datetime="<?php echo get_the_time('Y-m-d') ?>"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'birdfield' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php echo get_post_time( get_option( 'date_format' ) ); ?></a></span>
		<span class="icon author"><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"><?php the_author(); ?></a></span>
